...
let videoRecorder; // Define videoRecorder in a broader scope
let videoChunks = []; // Define videoChunks in a broader scope
...
ipcRenderer.on("init-videoRecorder", async () => {
  console.log("start init video...")
...
ipcRenderer.on("start-recording-video", async () => {
  try {
    updateWindowMessage("Recording video in progress...");
    const stream = await navigator.mediaDevices.getUserMedia({ video: true });
    videoRecorder = new MediaRecorder(stream); // Use the broader scoped mediaRecorder
    videoChunks = []; // Reset audioChunks for a new recording
    ...
    videoRecorder.addEventListener("stop", async () => {
      ...
      // Send the buffer to the main process
      ipcRenderer.send("video-buffer", buffer);
    });

    videoRecorder.start();
  } catch (error) {
    console.error("Error accessing the camera", error);
    updateWindowMessage("Failed to record video...");
  }
});
...
ipcRenderer.on("stop-recording-video", () => {
  // Stop the media recorder
  updateWindowMessage("Processing video...");
